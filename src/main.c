...
#include "frontend/parser/prism_parser.h"
...
static char* read_prism_source(const char* path) {
...
static error_t compile(Ctx ctx, ErrorsContext* errors, FileIoContext* fileio) {
    IdentifierContext identifiers;
    FrontEndContext frontend;
    BackEndContext backend;    unique_ptr_t(CProgram) c_ast = uptr_new();
    ...
    verbose(ctx, "-- Parsing ... ");
    {
        char* prism_src = read_prism_source(ctx->filename);
        if (!prism_src) {
            THROW_BASE(GET_UTIL_MSG(MSG_failed_fread, ctx->filename));
        }
        error_t prism_err = parse_prism(prism_src, ctx->filename, errors, &identifiers, &c_ast);
        free(prism_src);
        if (prism_err) {
            EARLY_EXIT;
        }
