...
function usage () {
    ...
    echo "    --freestanding  link without libc, crt or a dynamic linker"
    echo "    -T <script>     link with <script> as the linker script (implies --freestanding)"
    echo ""
    echo "[Include...]:"
...
function parse_link_arg () {
    case "${ARG}" in
        ...
        "-T")
            shift_arg
            if [ ${?} -ne 0 ]; then
                raise_error "missing filename after $(em "-T")"
            fi
            LD_SCRIPT="$(readlink -f ${ARG})"
            if [ ! -f "${LD_SCRIPT}" ]; then
                raise_error "cannot find $(em "${LD_SCRIPT}"): no such file"
            fi
            IS_FREESTANDING=1
            ;;
        *)
...
function add_linkdirs () {
    ...
    # Freestanding links must not pull in the bundled libc.
    if [ ${IS_FREESTANDING} -eq 0 ] && [ -d "${LIBC_DIR}" ]; then
...
# Bare metal: no crt, no libc, no dynamic linker. The kernel's entry point comes
# from the linker script's ENTRY(), not from _start in crt.s.
#
# -z max-page-size=0x1000 matters: ld defaults to 2MB alignment on x86-64, which
# pads the image out enormously for no benefit here.
function link_freestanding () {
    verbose "Link (ld, freestanding) -> ${NAME_OUT}"
    ld -m elf_x86_64 -nostdlib -static --no-dynamic-linker \
